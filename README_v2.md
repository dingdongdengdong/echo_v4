...
활짝        sweep 120.0 deg   bus 148.5   개구 57 mm
grasp 1.0   sweep  27.4 deg   bus  55.9   개구 0.5 mm   ← 정상 조작의 끝
최대 집기    sweep  21.2 deg   bus  49.7   ← 기계적 바닥, 여기 아래로는 안 감
```

#### 최대 집기 각도 (2026-09-02 실측)

`sweep 21.2 deg` = `bus 49.7 deg`. 닫는 방향으로 계속 밀어보며 잡았습니다.
조가 맞닿은 뒤로는 명령 대비 실제 이동이 줄어듭니다.

```
sweep 27.5 에서 −5.0° 명령 → 4.75° 이동 (95%)
sweep 22.8 에서 −2.0° 명령 → 1.58° 이동 (79%)   ← 저항 증가
```

**이건 작동점이 아니라 바닥입니다.** `GRIP_CLOSED_SWEEP_DEG`로 코드에 박아
`grasp_to_servo_deg()`가 이 아래를 절대 명령하지 못하게 했습니다. 정상 조작
범위는 그대로입니다 — VR 트리거를 끝까지 당기면 `grasp 1.0` = `sweep 27.4`
(개구 0.5 mm)까지만 갑니다.

`27.4` 아래는 개구가 아니라 **무는 힘**의 영역이라 `gripper_map`으로 표현할 수
없습니다 (서보 20°에서 개구가 −0.27 mm로 계산됨). 그래서 mm가 아니라 sweep 각도로
기록합니다.
